...
const DatePicker = ({ value, onChange, placeholder }) => {
  ...
  const getDateLabel = (dateString) => {
    ...
    const diffDays = Math.round(
      (Number(date) - Number(today)) / (1000 * 60 * 60 * 24)
    );
...
export const TrainSearch = ({ onSearch }) => {
  ...
  return (
    <div
      ...
      className="max-w-md mx-auto mt-10 bg-white rounded-lg overflow-hidden shadow-lg"
    >
      <div className="bg-gradient-to-r from-blue-900 to-blue-700 text-white flex">
        <button
          className={`flex-1 text-lg font-medium py-3 transition-colors ${
            activeTab === '单程' ? 'bg-white text-blue-900' : ''
          }`}
        ...
        <button
          className={`flex-1 text-lg font-medium py-3 transition-colors ${
            activeTab === '往返' ? 'bg-white text-blue-900' : ''
          }`}
      ...
      <div className="p-6">
        ...
        <div className="flex items-center mb-4 text-gray-600">
          ...
          />
          <label htmlFor="highSpeedOnly" className="ml-2">
            只看高铁动车
          </label>
        </div>
